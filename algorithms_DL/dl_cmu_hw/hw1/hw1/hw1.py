...
class SoftmaxCrossEntropy(Criterion):
    ...
    def forward(self, x, y):
        ...
        self.exp_scores = np.exp(x)
        self.sm = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)

        W1_r = 0.5 * np.sum(self.W * self.W)
		# W2_r = 0.5 * reg * np.sum(W2 * W2)
        self.loss = -np.log(probs[range(self.logits.shape[0]), self.labels])) / N
...
class MLP(object):
    ...
    def __init__(self, input_size, output_size, hiddens, activations, weight_init_fn, bias_init_fn, criterion, lr, momentum=0.0, num_bn_layers=0):
        ...
        self.W = random_normal_weight_init(input_size, output_size)
        self.dW = None
